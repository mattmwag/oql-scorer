<div>
    ...
    <table class="table table-bordered">
        <tr style="{{ $this->control == 1 && ($count % 4 != 0) ? 'background-color:yellow' : '' }}">
            ...
            <th>
                @if ($count % 4 != 0)
                <button wire:click="increment(1, 0, 10)" style="{{ $this->control == 1 ? 'color:red': '' }}">+10</button>
                <button wire:click="increment(1, 0, 5)" style="{{ $this->control == 2 ? 'color:red': '' }}">+5</button>
    ...
    <table class="table table-bordered">
        ...
        <th>{{ $team2->name }} {{ $this->control }}</th>
            <th>
                @if ($count % 4 != 0)
                <button wire:click="increment(2, 0, 10)" style="{{ $this->control == 2 ? 'color:red': '' }}">+10</button>
                <button wire:click="increment(2, 0, 5)" style="{{ $this->control == 1 ? 'color:red' : '' }}">+5</button>
